...
export function ExerciseCard(p: ExerciseCardProps) {
  ...
  return (
    <article className={`xcard${complete ? ' complete' : ''}`} id={`ex-${ex.key}`} aria-labelledby={`${feelId}-t`}>
      <div className="xhead">
        <ExerciseIllustration pose={ex.pose} size="lg" />
        <div className="tx">
          {complete
            ? <span className="done-head"><Icon name="check" />Ejercicio completado</span>
            : <span className="num">Ejercicio {p.number} de {p.total}</span>}
      ...
      <TechniquePanel ref={techRef} ex={ex} feel={p.feel} onFeel={p.onFeel} note={p.note} onSaveNote={p.onSaveNote} showFeel={!complete} />
    </article>
  );
}
