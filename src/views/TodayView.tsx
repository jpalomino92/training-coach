    ...
    <div className="today">
      ...
      <div className="list">
        {doneList.length > 0 && (
          <>
            ...
            {doneList.map(ex => (
              <ExerciseRow key={ex.key} ex={ex} number={day.exercises.indexOf(ex) + 1} sets={exSets(ex)} feel={workout?.feel?.[ex.key]} onOpen={() => openExercise(ex.key)} />
            ))}
          </>
        )}
        {active && (
        ...
        {nextList.length > 0 && (
          <>
            ...
            {nextList.map(ex => (
              <ExerciseRow key={ex.key} ex={ex} number={day.exercises.indexOf(ex) + 1} sets={exSets(ex)} onOpen={() => openExercise(ex.key)} />
            ))}
          </>
        )}
      </div>
      ...
      {workout?.status === 'completed' && (
        <div className="done-msg">
          <b><Icon name="check" />Entrenamiento completado el {fmtDate(workout.completed_at)}</b>
          <span className="sm">Puedes seguir editando las series si lo necesitas.</span>
          <button type="button" className="btn btn-link" style={{ alignSelf: 'flex-start', padding: 0 }} onClick={() => setTab('historial')}>Ver en historial</button>
        </div>
      )}
    </div>
  );
